...
import { FaceRetouchingNatural, RemoveRedEye } from '@mui/icons-material';
...
const exercises = [
  {
    ...
    description: 'Raise eyebrows, smile, frown, pucker lips',
    icon: <FaceRetouchingNatural sx={{ fontSize: 40, color: 'primary.main' }} />,
    path: '/therapy/bells/facial-movement'
  }
];
    ...
    <Layout>
      <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', py: 6 }}>
        ...
        <Typography variant="h6" align="center" sx={{ mb: 6, color: 'text.secondary' }}>
          Real-time facial movement and eye exercises with AI feedback
        </Typography>
          ...
          {exercises.map((ex, idx) => (
            <Grid item xs={12} md={5} key={ex.key}>
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}>
                <Card sx={{ borderRadius: 4, boxShadow: 6, p: 2, minHeight: 220, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #a8edea 0%, #fed6e3 100%)' }}>
